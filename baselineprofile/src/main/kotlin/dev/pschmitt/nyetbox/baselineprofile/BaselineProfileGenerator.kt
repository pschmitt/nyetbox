...
/**
 * Generates `app/src/main/baselineProfiles/baseline-prof.txt` (NBC-426) by exercising the
 * "cold start -> dashboard -> device list scroll -> device detail -> global search" journey the
 * perf audit called out as this app's steady-state usage pattern, against the `benchmark` build
 * type (release-derived, non-debuggable, profileable - see app/build.gradle.kts).
 *
 ...
 * lands straight on a populated Dashboard without driving onboarding through a live NetBox
 * instance or waiting on a real sync - see that receiver's doc comment for why.
 */
class BaselineProfileGenerator {
    ...
    fun generate() =
        rule.collect(packageName = TARGET_PACKAGE, includeInStartupProfile = true) {
            ...
            device.wait(Until.hasObject(By.res(TARGET_PACKAGE, "e2e-device-list-entry")), UI_TIMEOUT_MS)
